 ...
 */
public class GameRumah_2 implements IGameScene {
...
//                        System.out.println(player.getSprite().getFrame());
    ...
    public void pointerReleased(int x, int y) {
        ...
        if (finish != TypeList.PLAYING) {
            try {
                if (finish == TypeList.SUCCESS) {
                    if (GameDataHelper.getHighScore(GameDataHelper.BALAP_KARUNG_RUMAH_2) < player.getCoinCount()) {
                        GameDataHelper.writeHighScore(GameDataHelper.BALAP_KARUNG_RUMAH_2, player.getCoinCount());
                    }
    ...
    private void initLevel() {
        LevelGenerator.initConstraints(8, 12, 1, 2, 1, 2, 35, 50, speed);
        LevelGenerator.initDistance(50, 300, 150, 200);
        LevelGenerator.initObjective(TypeList.DISTANCE, 1500);
    ...
    private void renderScore(Graphics g) throws Exception {
        ...
        grImageHigh.drawString("HIGHSCORE :" + GameDataHelper.getHighScore(GameDataHelper.BALAP_KARUNG_RUMAH_2), 0, 0, Graphics.LEFT | Graphics.TOP);
