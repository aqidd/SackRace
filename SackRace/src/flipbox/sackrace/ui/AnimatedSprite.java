...
public class AnimatedSprite extends ChocoSprite{
    ...
    // Apakah AnimatedSprite ini di-flip
    boolean isFlipped;
    ...
    private void updateShowedFrame() {
        if(!isFlipped) {
            ...
            int frameIndex = this.getFrame() - 1;       
            if(frameIndex < 0 ){// jika sudah selesai satu sequence frame
                if( animateCounter!=ANIMATE_FOREVER && // dan bukan infinite loop
                        ++animateCounter==animateCount){ // dan jumlah loop habis)
                    isAnimate = false;  // Selesai animated    
                    timer.reset();
                } else {
                    this.setFrame(this.getFrameSequenceLength() - 1);
                }
            } else {
                this.prevFrame();   // Ganti gambar ke frame berikutnya
            }
        }
    }
    ...
    public void flip() {
        isFlipped = true;
        startIndex = this.getFrameSequenceLength() - 1;
    }
}
